...
import ru.classifier.client.thread.ProcessorUser;
...
public class Client {
  ...
  private final List BYTE_POINTS_IN_POOL = new LinkedList();
  private final List BYTE_POINTS_OUT_POOL = new LinkedList();
  ...
  private void initClient(final String teachFileName, final String dataFileName) {
    ...
    try {
      ...
      double [] r = { 1 * ((ParzenClass) c).getH() / 10.0 };
      ...
      for (int i = 0; i< dataMatrix.length; i++) {
        for (int j = 0; j< dataMatrix[i].length; j++){
          ...
          if (!HASH_MAP.containsKey(p)) {
            ...
            BYTE_POINTS_IN_POOL.notify();
          } else {
            doubles ++;
          }
        }
      }
      System.out.println("Client : Total points = " + total + "  repeated points = " + doubles);
  ...
  public static void  main( String args[] ) {
    ...
    if (true) {
      ...
      // local one thread mode
      client.classifyAllPointsLocally();
    }
  ...
  private void classifyAllPoints(final List processors) {
    ...
    for (Iterator i = processors.iterator(); i.hasNext();) {
      ...
      final ProcessorUser processorUser = new ProcessorUser(BYTE_POINTS_IN_POOL, BYTE_POINTS_OUT_POOL, processor);
      processorUser.start();
    }
